...
namespace AnilTools.Update
{
    public static class RegisterUpdate
    {
...
#else
            var task = new UpdateTask(action, endCnd, then);
#endif
        ...
        // updates till end of time
        // example: Time.time + 5
        // means till 5 second later
        public static UpdateTimeTask UpdateTill(this UnityEngine.Object sender, Action action, float endTime, Action then = null, UpdateType updateType = UpdateType.normal)
        {
            var task = new UpdateTimeTask(action, endTime, then, sender ? sender.GetInstanceID() : 0);
            AnilUpdate.Register(task, updateType);
            return task;
        }

        public static UpdateTaskGeneric<T> UpdateWhile<T>(this T sender, Action<T> action, Func<bool> endCondition, Action<T> then = null, UpdateType updateType = UpdateType.normal) 
        {
