...
namespace AnilTools
{
    public static class RandomReal
    {
        ...
        // küp gibi random
        public static Vector3 V3Randomizer(Vector3 value, int seed)
        {
            Random.InitState(seed);

            return new Vector3(Rangef(-value.x, seed), Rangef(new Vector3( -value.y / 2, value.y),seed), Rangef(value,seed));
        }

        public static float Rangef(this Vector2 minMax, int seed)
        {
            Random.InitState(seed);
            return Random.Range(minMax.x, minMax.y);
        }

        public static Vector3 V3Randomizer(float value)
        {
            Random.InitState((int)System.DateTime.Now.Ticks);

            return new Vector3(Rangef(-value, value), Rangef(-value / 2, value), Rangef(-value, value));
        }
        ...
        public static int Range(int x, int y, int lastValue)
        {
            IntChace = lastValue;
            ...
            {
                Random.InitState((int)System.DateTime.Now.Ticks + tryCount++);
                IntChace = Mathf.RoundToInt(Random.Range(x, y));
            }
